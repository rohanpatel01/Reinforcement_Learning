...
class ReplayBuffer:
    ...
    def store(self, experience_tuple):
        ...
        self.replay_buffer[self.__next_replay_location] = (state, action, reward, next_state, done)     # trying to use normal list but we want to count stull
        #
    ...
    def sample_minibatch(self):
        ...
        for i in range(self.config.minibatch_size):
            minibatch.append( buffer[ np.random.randint( self.__num_elements ) ] )         # len(self.replay_buffer)   self.__num_elements

        # Checking to see what minibatch contains

        # print("Inside minibatch:    ====================================")
        # print("Unique elements in replay buffer")
        # print("Number of elements: ", len(minibatch) )       #      len(minibatch)      len(self.replay_buffer)  <-- for set     self.__num_elements     <-- for list
        # counts = Counter(minibatch)                # minibatch    self.replay_buffer
